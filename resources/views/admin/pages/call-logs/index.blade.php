    ...
    <div class="card card-custom">
        <div class="card-header">
            ...
            <div class="card-toolbar">
                ...
                @endif --}}
                <a href="{{ route('admin.call-log.csv-download') }}"
        ...
        <div class="card-body">
            {{-- Datatable Start --}}
            <table class="table table-bordered table-hover table-checkable" id="call_logs_table"
...
<script>
    $(document).ready(function () {
        // datatable
        oTable = $('#call_logs_table').DataTable({
            responsive: true,
            searchDelay: 500,
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('admin.call-logs.listing') }}",
                data: {
                    columnsDef: ['creator_name', 'participator_name', 'date','remaining_time','action'],
                },
            },
            columns: [
                // { data: 'id' },
                { data: 'creator_name' },
                { data: 'participator_name' },
                { data: 'date' },
                { data: 'remaining_time' },
            ...
            columnDefs: [
                ...
                { targets: 0, title: "Creator Name", orderable: false },
                { targets: 1, title: "Participator Name", orderable: false },
                { targets: 2, title: 'Date', orderable: true },
                { targets: 3, title: 'Remaining Time', orderable: false },
                // Action buttons
