...
@push('breadcrumb')
    {!! Breadcrumbs::render('personality_create') !!}
@endpush
...
<div class="container">
    <div class="card card-custom">
        ...
        <!--begin::Form-->
        <form id="frmAddPersonality" method="POST" action="{{ route('admin.personalities.store') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <textarea type="text" class="form-control" id="{{ $language->getField($language->lang_code,'description') }}" name="{{ $language->getField($language->lang_code,'description') }}" placeholder="Enter {{ $language->hint }} description" autocomplete="{{ $language->getField($language->lang_code,'description') }}" spellcheck="false" autocapitalize="sentences" tabindex="0" autofocus>{{ old($language->getField($language->lang_code,'description')) }}</textarea>
            ...
            <div class="card-body">
                {{-- Image --}}
                <div class="form-group">
                    <label for="image">{!!$mend_sign!!} Image</label>
            ...
            <div class="card-footer">
                ...
                <a href="{{ route('admin.personalities.index') }}" class="btn btn-secondary text-uppercase">Cancel</a>
            </div>
        </form>
        <!--end::Form-->
    </div>
...
$(document).ready(function () {
    ...
    $('#frmAddPersonality').submit(function () {
        if ($(this).valid()) {
            addOverlay();
