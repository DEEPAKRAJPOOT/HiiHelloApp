        ...
        <form id="frmEditFaq" method="POST" action="{{ route('admin.faqs.update', $faq->custom_id) }}" enctype="multipart/form-data">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    id="{{ $language->getField($language->lang_code,'question') }}"
                    name="{{ $language->getField($language->lang_code,'question') }}"
                    value="@if(old($language->getField($language->lang_code,'question'))){{ old($language->getField($language->lang_code,'question')) }}@else{{ $faq->getValue($language->lang_code,'question') }}@endif"
                    placeholder="Enter {{ $language->hint }} question"
                    autocomplete="{{ $language->getField($language->lang_code,'question') }}"
                ...
                <div class="form-group">
                    ...
                    id="{{ $language->getField($language->lang_code,'answer') }}"
                    name="{{ $language->getField($language->lang_code,'answer') }}"
                    placeholder="Enter {{ $language->hint }} answer"
                    autocomplete="{{ $language->getField($language->lang_code,'answer') }}"
                    spellcheck="false" autocapitalize="sentences" tabindex="0" autofocus>@if(old($language->getField($language->lang_code,'answer'))){{ old($language->getField($language->lang_code,'answer')) }}@else{{ $faq->getValue($language->lang_code,'answer') }}@endif</textarea>
