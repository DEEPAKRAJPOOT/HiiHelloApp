@if(empty($deleted_entry))
    ...
    @endif
@else
    @if(in_array('restore', $permissions) && !empty($restorable))
        <button data-target-href="{{ route($routeName.'.restore', $id) }}" class="btn btn-icon btn-light btn-hover-success btn-sm action-restore" title="Restore {{ $custom_title ?? '' }}">
