...
class CheckPermit
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        // Permissions 
        $access = ['index', 'listing', 'showSetting', 'change-setting', 'trashed', 'trasheddata','actor-list','singer-list','unde-review','genderupdate','bulk_gender_update','bulk-approve','dashboardupdate','bulk_photo_verification','bulk_email_verification','deleted','filters','under-review-listing','refund-transaction'];
        $add = ['store', 'create'];
        $update = ['edit', 'update'];
        $view = ['show'];
        $delete = ['destroy'];
