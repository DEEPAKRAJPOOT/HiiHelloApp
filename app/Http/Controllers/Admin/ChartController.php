...
class ChartController extends Controller
{
    public function getRegisterUser(Request $request)
    {
        ...
        foreach ($period as $date) {
            ...
            $data[] = User::whereBetween('created_at', [Carbon::parse($date->startOfDay()), $date->endOfDay()])->count();
        }


        // $users = User::select('created_at')->whereBetween('created_at', [$startDate, $endDate])->orderBy('created_at')->get()->groupBy(function ($user) use ($dateFormat) {
