    ...
    public function handle()
    {
        ...
        User::select('id','custom_id','email','verify_photo_status','verify_email_send','email_verified_at','contact_verified_at')
                ...
                ->chunk(10000, function($users) {
                        if($users->isNotEmpty())
                        {
                            $pre_month_start_date = date('Y-m-d', strtotime("-30 days")); 
                            $pre_month_end_date = date("Y-m-d");
                            ...
                            foreach($users as $user)
                            {
                                ...
                                // If Trusted Score is less than 1 default will be 1
                                if($Trusted_Score_Total < -1){
                                    $Trusted_Score_Total = 1;
                                }
